...
$query_pinjaman="SELECT * FROM tbl_pinjaman_anggota WHERE id_pinjaman='$id'";
...
if ( $jp > $data_pinjaman['jumlah_pinjaman'] ){
    ...
    document.location.href='dashboard.php?p=data_pinjaman_anggota&id=".$id."'</script>\n";
}else{

      $query1 = "INSERT INTO `tbl_pembayaran_pinjaman` (`id_pembayaran`, `id_anggota`, `periode`, `tahun`, `tgl_pembayaran`, `jumlah_pinjaman`, `jml_angsuran_bunga`, `jenis_bunga`, `bunga`, `jangka_waktu`, `angsuran_k`, `sisa_angsuran`, `angsuran_pokok`, `angsuran_bunga`, `total_angsuran`, `total_tunggakan`) VALUES ('$idpembayaran', '$np', '$bulan', '$tahun', '$tp', '$jp', '', '$jb', '$bunga', '$jw', '$ak', '$sa', '$ap', '$ab', '$ta', '$tt')";
      $sql1 = mysqli_query($connect, $query1); 
      if ($sql1) {
        $query_r="UPDATE tbl_pinjaman_anggota SET jumlah_pinjaman='$jumlahpinjaman' WHERE id_pinjaman='$id'";
               $sql_r=mysqli_query($connect, $query_r);
               if ($query_r) {
                $query_a="UPDATE tbl_pinjaman_anggota SET jangka_waktu='$jangkawaktu' WHERE id_pinjaman='$id'";
               $sql_a=mysqli_query($connect, $query_a);
               if ($query_a) {
                $queryb="UPDATE tbl_pinjaman_anggota SET status='$status' WHERE id_pinjaman='$id'";
