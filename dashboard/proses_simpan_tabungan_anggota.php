...
$jma=$data['ttl_tabungan'];
$jma=$tt;
if ($result > 0) {
	$query1 = "INSERT INTO `tbl_tabungan_anggota` (`id_tabungan_a`, `id_anggota`, `periode_a`, `tahun_a`, `tanggal_setor`, `nama_penyetor`, `jumlah_tabungan`, `bunga_ta`, `jml_bunga`, `total_tabungan`) VALUES ('$idtabungana', '$penerima', '$bulan', '$tahun', '$tgl', '$penyetor', '$jta', '$bunga', '$jb', '$tt')";
	...
	$sql1 = mysqli_query($connect, $query1); 
	if ($sql1) {
		$query2 = "INSERT INTO `tbl_rekap_tabungan_anggota` (`id_ra`, `id_anggota`, `periode_a`, `tahun_a`, `jml_bunga`, `ttl_tabungan`) VALUES ('', '$penerima', '$bulan', '$tahun', '$jb', '$jma')";
		$sql2 = mysqli_query($connect, $query2); 
		if ($sql2) {
			echo "<script>alert('Data Tabungan Anggota Berhasil Disimpan');
		document.location.href='dashboard.php?p=data_tabungan_anggota'</script>\n";
